...
# 이 함수는 순환적으로 호출된다.
call_count = 0
def alphabeta(board, depth, maxPlayer, a = -99999,b = 99999):
    global call_count
    call_count += 1
    pos = -1
    ...
    if maxPlayer:
        ...
        for p in empty_cells(board):
            ...
            # 경기자를 교체하여서 alphabeta()를 순환호출한다. 
            x, score = alphabeta(board, depth-1, False, a,b)
        ...
        for p in empty_cells(board):
            ...
            # 경기자를 교체하여서 alphabeta()를 순환호출한다. 
            x, score = alphabeta(board, depth-1,True,a,b)
            board[p] =  ' '		# 보드는 원 상태로 돌린다. 
            if score < value:
...
while True:
    ...
    if len(empty_cells(game_board)) == 0 or game_over(game_board):
        break
    i, v = alphabeta(game_board, 9, player=='X')
    move(i, player)
    if player=='X':
        player='O'
    else: 
